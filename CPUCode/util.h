...
public:
    ...
    LSH();
    LSH(int din, int dout);

    void set_dimensions(int din, int dout);
    unsigned int get_out_dimension() const;
    
    // randomly generate the LSH functions
    void generate();
    void test();
...
private:
    ...
    // the random float number generated by the cstdlib is too small
    // we multiply the number by NULTIPLIER to get the final hashing function
    static constexpr float MULTIPLIER = 1000.0f;
};
