...
namespace clustering
{
    ...
    void DBSCAN::UnionFind::print_union(){
    ...
    int DBSCAN::merge_in_neighbour(const DBSCAN::ClusterData& cl_d, int point_id, int center_id, DBSCAN::UnionFind& uf, const std::unordered_map<int, int>& reverse_find){
        static const int num_neighbour = 21;
        int cell_iter = center_id - 2 * (m_n_cols + 1) - 1;
    ...
    void DBSCAN::merge_clusters(const DBSCAN::ClusterData& cl_d){
        ...
        for(std::unordered_map<int, std::vector<int> >::const_iterator iter = m_hash_grid.begin(); iter != m_hash_grid.end(); ++iter){
            ...
            for(unsigned int i=0; i<iter->second.size(); i++){
                ...
                belong_id = merge_in_neighbour(cl_d, point_id, cell_id, uf, reverse_find);
                //if(uf.find(belong_index) != uf.find(cell_index)){
