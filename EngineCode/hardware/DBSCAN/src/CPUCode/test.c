...
void DBSCAN_cpu(int num_of_points, float eps, float* arr, float* answer){
    int length = num_of_points * 2;
...
void test_cpu(){
    ...
    float eps = 0.1;
    DBSCAN_cpu(num_of_points, eps, arr, answer_cpu);
}
...
void test_dfe(){
    ...
    answer_dfe = (uint8_t*)safe_malloc(sizeof(uint8_t) * num_of_points * 3);
    float eps = 0.1;
    DBSCAN(num_of_points, eps, arr, answer_dfe);
}
